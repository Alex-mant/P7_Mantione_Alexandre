...
export const arrayDoubleTreatment = (array, newArray, target) => {
    array.map((el) => {
        if (Array.isArray(el[target])){ //Si c'est un tableau dans un tableau
            if (el[target].some(value => {return typeof value == "object"})){ // si c'est un objet dans un tableau qui est dans un tableau
                el[target].forEach((obj) => {
                    newArray.push(obj[Object.keys(obj)[0]].charAt(0).toUpperCase()+obj[Object.keys(obj)[0]].slice(1));
                })
            }else{ //Si c'est un tableau dans un tableau
              el[target].forEach(elChild => {
                newArray.push(elChild.charAt(0).toUpperCase()+ elChild.slice(1))
                });  
            }            
        }else if (typeof(el[target]) === "string") { //Si c'est un objet dans un tableau
